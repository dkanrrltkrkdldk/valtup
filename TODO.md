...
- [x] 포인트 만료 통계 스케줄러 (매일 01:00 KST, 만료 포인트 로깅)
- [x] 예산 자동 생성 스케줄러 (매일 00:00 KST, 기본 100,000p)
...
- [x] **내 포인트 페이지** (`/points`)
  - [x] 포인트 목록 (유효기간 표시)
  - [x] 만료된 포인트 상태 표시
  - [x] 7일 내 만료 예정 포인트 알림/하이라이트
  - [x] 총 잔액 표시
- [x] **상품 목록 페이지** (`/products`)
  - [x] 상품 카드 그리드
  - [x] 내 포인트로 구매 가능 여부 표시
  - [x] 구매 버튼/모달
- [x] **주문 내역 페이지** (`/orders`)
  - [x] 주문 목록
  - [x] 주문 상태 표시
...
- [x] 레이아웃 (사이드바 네비게이션)
- [x] UI 컴포넌트 라이브러리 설정
- [x] 어드민 인증 처리 (role 체크, 권한 없으면 접근 차단)
...
- [x] **주문 내역** (`/orders`)
  - [x] 주문 목록 테이블
  - [x] 주문 상태 필터 (전체/완료/취소됨)
  - [x] 주문 취소 버튼 (포인트 환불)

### 4.3 E2E 테스트
- [x] Playwright 설정
- [x] Page Object Models (6 pages)
- [x] E2E 테스트 작성 (38 tests)
  - [x] 인증 테스트 (7 tests)
  - [x] 대시보드 테스트 (5 tests)
  - [x] 예산 관리 테스트 (6 tests)
  - [x] 상품 관리 테스트 (10 tests)
  - [x] 주문 내역 테스트 (10 tests)
...
### 5.1 기본 구현
- [x] WebView 설정 (webview_flutter)
- [x] 사용자 웹 URL 로드
- [x] 뒤로가기 처리 (WebView 내 히스토리)
- [x] 로그인 상태 유지 (쿠키/세션 유지)

### 5.2 추가 구현 (가산점)
- [x] 앱 아이콘 변경 (설정 완료, PNG 변환 필요)
- [x] 앱 이름 변경 ("Point Roulette")
- [x] 네트워크 에러 처리 (커스텀 에러 페이지 + 재시도)
- [x] 로딩 인디케이터 (스피너)
